...
`File`直接继承于`Object`，实现了`Serializable`接口和`Comparable`接口，意味着`File`对象支持序列化操作。而实现`Comparable`接口，意味着`File`对象之间可以比较大小；`File`能直接被存储在有序集合(如TreeSet、TreeMap)中。
...
```java
// 新建目录 /home/hugh/dir
File dir = new File("/home/hugh/dir");
dir.mkdir();
```

>说明：
>
>上面是在linux系统下新建目录"/home/hugh/dir"的源码。
>
>在Windows下若要新建目录"D:/dir"，源码如下：
...
```java
URI uri = new URI("file:/home/hugh/dir");
...
> 说明：
>
>  和方法2类似，只不过方法2中传入的是完整路径，而方法3中传入的是完整路径对应URI。

## 新建子目录的常用方法

例如，我们想要在当前目录的子目录dir下，再新建一个子目录。有一下几种方法:
...
说明：上面的方法作用是，在当前目录下创建`sub1`。它能正常运行的前提是`sub1`的父目录`dir`已经存在。
...
说明：上面的方法作用是：在当前目录下创建`sub2`。它能正常运行的前提是：`sub2`的父目录`dir`已经存在。
...
说明：上面的方法作用是，在当前目录创建`dir/sub3`。它不需要`dir`已经存在，也能正常运行；若`sub3`的父目录不存在，`mkdirs()`方法会自动创建父目录。

方法4

```java
File sub4 = new File("/home/hugh/dir/sub4");
sub4.mkdir();
```

说明：上面的方法作用是，新建目录`/home/hugh/dir/sub3`。它不需要`dir`已经存在，也能正常运行；若`sub4`的父目录不存在，`mkdirs()`方法会自动创建父目录。

方法5

```java
URI uri = new URI("file:/home/hugh/dir/sub5");
File sub5 = new File(uri);
sub5.mkdir();
```

说明： 和方法4类似，只不过方法4中传入的是完整路径，而方法5中传入的是完整路径对应URI。

## 新建文件的常用方法

例如，我们想要在当前目录的子目录`dir`下，新建一个文件。有以下几种方法
...
说明：说明代码的作用是在`dir`目录(相对路径)下新建文件`file2.txt`。

方法3

```java
try {
    File file3 = new File("/home/hugh/dir/file3.txt");
    file3.createNewFile();
} catch (IOException e) {
    e.printStackTrace();
}
...
说明：上面代码作用是，新建文件`/home/hugh/dir/file3.txt`(绝对路径)。这是在linux下根据绝对路径的方法，在windows下可以通过以下代码新建文件`D:/dir/file4.txt`。
...
```java
try {
    URI uri = new URI("file:/home/hugh/dir/file4.txt"); 
...
说明：和方法3类似，只不过方法3中传入的是完整路径，而方法4中传入的是完整路径对应URI。
...
运行结果(在ubuntu系统下的运行结果，而不是windows！)：
...
File.separatorChar="/"
   ...
   s1_normal.getAbsolutePath() = /home/hugh/wind_talker/workout/java/hugh/io/io/src/file/dir/sub1/s1_normal.txt
  s1_normal.getCanonicalPath() = /home/hugh/wind_talker/workout/java/hugh/io/io/src/file/dir/sub1/s1_normal.txt
      s1_normal.lastModified() = 1595169126738 is "2020-07-19 22:32:06"
             s1_normal.toURI() = file:/home/hugh/wind_talker/workout/java/hugh/io/io/src/file/dir/sub1/s1_normal.txt
