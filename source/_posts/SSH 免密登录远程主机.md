---
title: Github Action 部署 hexo 博客
...
![](https://www.bing.com/th?id=OHR.ReussRiver_ZH-CN5897721217_1920x1080.jpg&rf=LaDigue_1920x1080.jpg&pid=hp)

<!--more-->

天下苦 ` hexo clean && hexo g $ hexo d`久已。

每次想要发一篇博客都要执行这些命令，还要填写各种密码，实在是麻烦。

遂各处搜罗解决方案，得此一文[使用GitHub Action自动部署Hexo](https://inkdp.github.io/skill/front-end/3295.html)，不禁感慨大自然的鬼斧神工。😜
...
- [密钥是什么](https://wangdoc.com/ssh/key.html#密钥是什么)
- [密钥登录的过程](https://wangdoc.com/ssh/key.html#密钥登录的过程)
- `ssh-key`命令：生成密钥
  - [基本用法](https://wangdoc.com/ssh/key.html#基本用法)
  - [配置项](https://wangdoc.com/ssh/key.html#配置项)
- [手动上传公钥](https://wangdoc.com/ssh/key.html#手动上传公钥)
- [ssh-copy-id 命令：自动上传公钥](https://wangdoc.com/ssh/key.html#ssh-copy-id-命令：自动上传公钥)
- ssh-agent 命令，ssh-add 命令
  - [基本用法](https://wangdoc.com/ssh/key.html#基本用法-1)
  - [`ssh-add`命令](https://wangdoc.com/ssh/key.html#ssh-add命令)
- [关闭密码登录](https://wangdoc.com/ssh/key.html#关闭密码登录)
...
开发过程中经常需要通过`SSH`协议登录到服务器做一些部署测试环境、查看日志等操作，需要记住很多服务器的账号和密码。密码设置的太简单了，不安全；密码设置的太复杂了，记不住。

相信大家都使用过`Git`以及`Github`，不知道你是否还记得，你曾经在本地使用`ssh-keygen`生成一对秘钥，然后将公钥（`~/.ssh/id_rsa.pub`）添加到`Github`网站上，在这个操作之前，每次提交代码都需要输入密码，烦不胜烦。同理，登录服务器应该也可以做到免密。
...
大功告成，现在再通过`ssh user@server`登录服务器时就不需要输入密码了。
...
基本上按照[这里](https://inkdp.github.io/skill/front-end/3295.html)的步骤就可以实现部署，这里主要记录一下踩的坑奥。



首先创建一个`deploy.yml`，`${{ secrets.CI_TOKEN }}`设置Github Token。`${{ secrets.ACTIONS_DEPLOY_KEY }}`设置你的公钥。
    ...
    steps:
      ...
      - name: 部署到个人服务器
        ...
        run: |
          ...
          chmod 600 ~/.ssh/id_rsa
          # 将public 里的东西发布到 SERVER_DIR 下的 hexo文件夹，注意修改成你需要的路径
          mv ./public ./hexo
          scp -o StrictHostKeyChecking=no -r ./hexo $SERVER_DIR
```

**小声嘀咕**：注意 Github 中 secrets 设置的字符要与`deploy.yml`中的对应，否则读不到值还不容易发现，毕竟我在这找了一天，还是[老哥](https://github.com/InkDP)帮忙发现的
